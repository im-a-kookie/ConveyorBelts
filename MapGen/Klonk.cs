...
public struct Klunk
{
    ...
    {
        return (Prefixes[(val - 1) & 15], Bodies[(val - 1) & 15], Suffixes[(val - 1) & 15]);
    }
...
public class Klonk
{
    ...
    public static IRenderer XNARenderer = null;
    public static IRenderer NETRenderer = null;
    static Klonk()
    {
        string @namespace = "Microsoft.XNA.Framework.Graphics";
        string @class = "SpriteBatch";
        var myClassType = Type.GetType(String.Format("{0}.{1}", @namespace, @class));
        if (myClassType != null)
        {
            XNARenderer = new MonogameRenderer();
        }
        @namespace = "System.Drawing";
        @class = "Graphics";
        myClassType = Type.GetType(String.Format("{0}.{1}", @namespace, @class));
        if (myClassType != null)
        {
            NETRenderer = new GDIRenderer();
        }
    ...
    public void Render(dynamic g, int x, int y, int size = 2)
    {
        int a = x;
        int b = y;
        for (int i = 0; i < body.Length; i++)
    ...
    {
        public void DrawText(dynamic renderer, Klonk k, int x, int y, int size, dynamic color);
    }
                ...
                else
                {
                    for (int j = 0; j < u.Length; j++)
                    {
                        ...
                        renderer.DrawImage(null,
                           new Rectangle(u[j].X + a, u[j].Y + b, u[j].W, u[j].H),
                           new Rectangle(u[j].U, u[j].V, u[j].W, u[j].H), 2);
                        //stepX = int.Max(stepX, u[j].W);
                    }
                }
                a += stepX + (2 * size) / 3;
    ...
    public class MonogameRenderer : IRenderer
    {
        ...
        public void DrawText(dynamic renderer, Klonk k, int x, int y, int size, dynamic c)
        {

            //calculate x/y offset for the UV map
            //size is given in per-character width (4/6/8/...)
            if (size < 4) size = 4;
            int index = (size / 2) - 2;
            ...
            if (index > offsets.Length)
            {
                ...
                //now scale up to the required thing
                int pxSize = (index + 2) * 2;
                //so we can scale up
                scale = (float)size / (float)pxSize;
            }

            (uxOff, uyOff) = offsets[index];
            ...
            for (int i = 0; i < k.body.Length; i++)
            {
                ...
                else
                {
                    for (int j = 0; j < u.Length; j++)
                    {
                        ...
                        renderer.Draw(MainTexture,
                            new Microsoft.Xna.Framework.Rectangle(u[j].X + a, u[j].Y + b, u[j].W, u[j].H),
                            new Microsoft.Xna.Framework.Rectangle(u[j].U, u[j].V, u[j].W, u[j].H), 
                            c);
