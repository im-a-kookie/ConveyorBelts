...
{
    /// <summary>
    /// A caching/wrapper class for the vertex buffers. Allows us to minimize buffer recreation.
    /// <para>Dynamic equivalent of <see cref="VertexBufferWrapper"/></para>
    /// </summary>
    public class DynamicVertexBufferWrapper
    {
        /// <summary>
        ...
        /// </summary>
        public DynamicVertexBuffer _buffer;
        /// <summary>
        /// The index buffer built from this wrapper
        /// </summary>
        public DynamicIndexBuffer _indices;
        /// <summary>
        ...
        public DynamicVertexBufferWrapper(int tw, int th)
        {
            TextureSize = new Point(tw, th);
        }

        public DynamicVertexBufferWrapper(int size)
        {
        ...
        /// <para>This inherently caches the buffers, which makes it very performant.</para>
        /// <para>This caching is the secret sauce that lets us surpass the SpriteBatch.</para>
        /// </summary>
        /// <returns></returns>
        public (VertexBuffer vertices, IndexBuffer indices) ConstructBuffer()
        {
            ...
            //    _buffer = new DynamicVertexBuffer(Core.Instance.GraphicsDevice, typeof(VertexPositionColorTexture), vertex_count, BufferUsage.WriteOnly);
            //}
            if (_buffer != null) _buffer.Dispose();
            _buffer = new DynamicVertexBuffer(Core.Instance.GraphicsDevice, typeof(VertexPositionColorTexture), vertex_count, BufferUsage.WriteOnly);
            ...
            //    _indices = new DynamicIndexBuffer(Core.Instance.GraphicsDevice, IndexElementSize.ThirtyTwoBits, index_count, BufferUsage.WriteOnly);
            //}
        ...
        {
            //preemptively rent an array using a default value (TODO: move to config)
            if (indices == null) indices = ArrayPool<int>.Shared.Rent(192);
            //Grow before we run out of spaces.
            if (index_count + 5 >= indices.Length)
            {
                //Doubling is probably good enough
                int[] _new = ArrayPool<int>.Shared.Rent(indices.Length << 1);
                Array.Copy(indices, _new, indices.Length);
                ArrayPool<int>.Shared.Return(indices);
                indices = _new;
            }
            //Follow the same logic as above for vertices
        ...
        /// <summary>
        /// Draw this vertex wrapper to the given graphics context
        /// </summary>
