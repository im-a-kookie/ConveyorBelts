...
namespace GrassHelper
{
    internal class NoiseGen
    {
        ...
        public class Simplex
        {
            ...
            static float[] _weights = [0.13f, 0.15f, 0.2f, 0.25f, 0.3f, 0.5f];

            float[] weights;

            public Simplex(int seed = 0)
            {
                Seed = seed;
                ...
                for (var i = 0; i < width; i++)
                {
                    for (var j = 0; j < height; j++)
                    {
                        ...
                        for (int k = 0; k < _weights.Length; ++k)
                        {
                            ...
                            v += (Generate(i * f, j * f) * 0.5f + 0.5f) * weights[k];
                            f /= 2f;
                        }
                        values[i, j] = v;
                    }
            ...
            public static Vector2 randomGradient(int ix, int iy)
            {
                ...
                Vector2 v;
                v.X = float.Cos(random); v.Y = float.Sin(random);
                return v;
            }
            ...
            public static float[,] GeneratePerlinNoise(int width, int height, float frequency, int octaves)
            {
                ...
                for (int y = 0; y < height; y++)
                {
                    for (int x = 0; x < width; x++)
                    {
                        ...
                        {
                            float sampleX = x / f * amplitude;
                            float sampleY = y / f * amplitude;
